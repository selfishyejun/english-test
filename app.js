...
const state={type:null,selected:new Set(),sort:'ordered',orderBlocks:'auto',openMocks:new Set(),openLessons:new Set(),session:[],index:0,lastSelection:[],lastWrong:[]};
const byId=new Map(SOURCE_DATA.map(x=>[x.id,x]));
const views=['landingView','setupView','quizView','resultView'];
...
function partitionCandidates(sentences,k,relaxed=false){
  ...
  function evaluate(cuts){
    ...
    groups.forEach(g=>{if(DEPENDENT_START.test(g[0]))score+=7;if(g.length===1)score+=4;});
...
function makeOrder(item){
  ...
  for(const relaxed of [false,true]){
    for(const k of counts){
      const c=partitionCandidates(item.sentences,k,relaxed);
      if(c.length){chosenCandidates=c;break;}
    }
    if(chosenCandidates)break;
  }
  if(!chosenCandidates)return null;
  const chosen=chosenCandidates[Math.floor(Math.random()*Math.min(5,chosenCandidates.length))];
  ...
  return{kind:'order',source:item,lead:item.sentences[0],blocks:shown,answer,user:'',blockCount:shown.length};
}
...
function renderQuiz(){const q=state.session[state.index];if(!q)return;const p=state.session.length?((state.index+1)/state.session.length*100):0;$('progressText').textContent=`${state.index+1} / ${state.session.length}`;$('progressBar').style.width=p+'%';$('prevBtn').disabled=state.index===0;$('nextBtn').disabled=state.index===state.session.length-1;$('mainAction').textContent=state.index===state.session.length-1?'채점하기':'답안 저장 · 다음';const item=q.source;const meta=`${item.label}`;let html=`<div class="meta"><div class="source-label">${escapeHtml(meta)}</div><div class="topic">${escapeHtml(item.title||'')}</div></div>`;if(q.kind==='order'){const allowed=LETTERS.slice(0,q.blocks.length);const example=q.blocks.length===5?'CEADB':q.blocks.length===4?'BCAD':'BCA';html+=`<h2 class="question-prompt">주어진 글 다음에 이어질 글의 순서로 가장 적절한 것을 고르시오.</h2><div class="lead">${escapeHtml(q.lead)}</div><div class="order-list">`+q.blocks.map(b=>`<div class="order-part"><div class="label">(${b.label})</div><div>${escapeHtml(b.text)}</div></div>`).join('')+`</div><div class="answer-zone"><label for="orderInput">정답 입력</label><input id="orderInput" class="manual-input" maxlength="${q.blocks.length}" autocomplete="off" placeholder="예: ${example}" value="${escapeHtml(q.user||'')}"><div class="input-help">${allowed.join(', ')}를 한 번씩 직접 입력하세요. Enter를 누르면 저장 후 다음 문제로 넘어갑니다.</div></div>`;}else{html+=`<h2 class="question-prompt">주어진 문장이 들어가기에 가장 적절한 곳을 고르시오.</h2><div class="insert-sentence">${escapeHtml(q.target)}</div><div class="insertion-text">${renderInsertion(q)}</div><div class="answer-zone"><label>선택한 위치</label><div class="input-help" style="font-size:12px">${q.user?q.user+'번':insertionChoiceRange(q)+' 중 하나를 클릭하세요.'}</div></div>`;}$('quizPaper').innerHTML=html;if(q.kind==='order'){const inp=$('orderInput');const allowed=LETTERS.slice(0,q.blocks.length);inp.focus();inp.addEventListener('input',()=>{let v=inp.value.toUpperCase().split('').filter(c=>allowed.includes(c)).join('');let uniq='';for(const c of v)if(!uniq.includes(c))uniq+=c;inp.value=uniq.slice(0,q.blocks.length);q.user=inp.value;});inp.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();mainAction();}});}else document.querySelectorAll('.gap').forEach(btn=>btn.onclick=()=>{q.user=btn.dataset.num;renderQuiz();});}
...
function mainAction(){const q=state.session[state.index];if(q.kind==='order'&&q.user.length!==q.blocks.length){$('orderInput')?.focus();return}if(q.kind==='insertion'&&!q.user)return;if(state.index===state.session.length-1)grade();else{state.index++;renderQuiz();}}
...
document.querySelectorAll('.type-btn').forEach(btn=>btn.onclick=()=>{state.type=btn.dataset.type;state.selected.clear();renderSetup();show('setupView');});
$('changeType').onclick=()=>show('landingView');
